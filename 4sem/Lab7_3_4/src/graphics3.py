...
def error_of_number_of_nodes(fun, left, right):
    plt.figure(1)
    plt.title(f"Error of number of nodes \n interval = [{left,right}]")
    Integral = integration3_8(fun, left, right, eps=1e-15)
    print("error of number of nodes", Integral)
    ...
    plt.ylabel('error')
    plt.savefig(f'..\\images_3_8\\error_of_number_of_nodes_[{left,right}].png')
...
def number_of_nodes_of_defined_eps(fun, left, right):
    ...
    print("number of nodes of defined eps", eps_arr, number_of_nodes)
    plt.grid()
    # plt.yscale('log')
    plt.xscale('log')
    plt.xlabel('eps')
    plt.ylabel('number of nodes')
    plt.savefig(f'..\\images_3_8\\number_of_nodes_of_defined_eps_[{left,right}].png')


def compare_tolerance_eps_and_exact_eps(fun, left, right):
    plt.figure(3)
    plt.title(f"Compare tolerance eps and exact epsilon \n interval = [{left,right}]")
    eps = 1e-5
    exact_eps_arr = []
    tolerance_eps_arr = []
    for i in range(1, 11):
    ...
    plt.xlabel('eps')
    plt.ylabel('number of nodes')
    plt.savefig(f'..\\images_3_8\\compare_tolerance_eps_and_exact_eps_[{left,right}].png')


def order_of_convergence(fun, left, right):
    plt.figure(4)
    plt.title(f"Identify an order of convergence. \n interval = [{left,right}]")
    eps = 1e-15
    Integral = integration3_8(fun, left, right, eps)
    p = []
    i_arr = []
    ...
    plt.ylabel('p')
    plt.savefig(f'..\\images_3_8\\order_of_convergence_[{left,right}].png')
